...
// cargar items existentes (si no existe usa vacío)
$items = [];
...
// lista de productos existentes para el select + datos
...
// obtener categorías para el select (si existen)
...
// procesar formulario agregar item
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Si el usuario seleccionó un producto existente, su id viene en selected_product_id
    ...
    // manejar archivo subido (si lo hay)
    $fotoNombre = '';
    ...
    // Si no se subió nueva imagen pero se seleccionó un producto existente, usar su foto
    if ($fotoNombre === '' && $selected_product_id > 0) {
        foreach ($productos as $p) {
            if ((int)$p['product_id'] === $selected_product_id) {
                $fotoNombre = $p['foto'] ?? '';
                // si el nombre del producto está vacío usar el del producto seleccionado
    ...
    header("Location:carrusel.php?mensaje=Agregado");
    exit;
}
    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <h2 class="mb-4 text-primary">
                <i class="bi bi-images"></i> Administrar Carrusel
            </h2>

            <div class="card shadow-lg border-0 mb-4">
                <div class="card-header bg-primary text-white py-3">
                    <h5 class="mb-0">Agregar productos al carrusel</h5>
                </div>
                ...
                <div class="card-body p-4">
                    <form action="carrusel.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="select_product" class="form-label fw-bold">
                                <i class="bi bi-search"></i> Seleccionar producto existente:
                            </label>
                            ...
                            <small class="form-text text-muted d-block">Al seleccionar, los demás campos se autocompletan.</small>
                            <!-- campo oculto para guardar id del producto seleccionado -->
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control form-control-lg" name="product_name" id="product_name" placeholder="Nombre del producto" required/>
                        </div>

                        <!-- Foto preview y carga -->
                        <div class="mb-4">
                            ...
                            <input type="file" class="form-control form-control-lg" name="foto" id="foto" accept="image/*"/>
                            <small class="form-text text-muted d-block mt-2">Si subes imagen se usará esa; si no, se usará la imagen del producto seleccionado.</small>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" class="form-control form-control-lg" name="price" id="price" placeholder="0.00"/>
                        </div>

                        <div class="d-flex gap-3 justify-content-end mt-4">
                            <a href="index.php" class="btn btn-secondary btn-lg"><i class="bi bi-x-circle"></i> Cancelar</a>
                            <button type="submit" class="btn btn-success btn-lg"><i class="bi bi-check-circle"></i> Agregar al carrusel</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h5>Productos del carrusel</h5>
                </div>
                <div class="card-body">
                    <?php if (empty($items)): ?>
                        <div class="alert alert-info">No hay items en el carrusel.</div>
                    <?php else: ?>
                        <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="d-flex gap-3 align-items-center border rounded p-3">
                                        ...
                                        <div class="flex-grow-1">
                                            ...
                                            <p class="mb-1">Categoria: <?php echo htmlspecialchars($it['category_name'] ?? ''); ?></p>
                                            <p class="mb-0">Precio: $<?php echo number_format((float)($it['price'] ?? 0),2); ?></p>
                                        </div>
                                        <div>
                                            <a href="carrusel.php?idEliminar=<?php echo (int)$it['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Eliminar item?')">Eliminar</a>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
...
<script>
    // datos de productos para autollenar (sacados desde PHP)
    ...
    document.getElementById('select_product').addEventListener('change', function() {
        ...
        const selectedField = document.getElementById('selected_product_id');
    ...
    // si el usuario carga una foto manualmente mostrar preview
